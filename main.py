...
class Point:
    def __init__(self, position, parent, orientation, distance, move=None, parent_distance=None):
        ...
        for jump in range(7):
            if abs(jump - self.move) <=1:
                if jump == 3:
                    ...
                    sl_points.append(Point((int(x) + self.x, int(y) + self.y), self.position, orient, 2.5, jump, self.distance))
                else:
                    ...
                    sl_points.append(Point((int(x) + self.x, int(y) + self.y), self.position, orient,
                                           abs(jump - 3) * 2 + 4, jump, self.distance))
        return sl_points
...
if map_name == "mapa6.png":
    start_pos = (20, 20)
    end_pos = (40, 260, 180)
    start = Point(start_pos, None, 0, 0, 3)
...
while queue:
    ...
    for new_point in new_points:
        ...
        if new_point.move == 3:
            ...
            for i in range(1, 20):
                ...
                if map[middle_point[0] + int(x_add), middle_point[1] + int(y_add), 0] == 0 or \
                    ...
                    is_new = False
                    break
        if 0 > new_point.x or new_point.x > height-5 or new_point.y > width-5 or 0 > new_point.y:
            is_new = False
            continue
        ...
        if is_new:
            ...
            if abs(new_point.x - end_pos[0]) < 20 and abs(new_point.y - end_pos[1]) < 20 \
                    and abs(new_point.orientation % 360) == end_pos[2]:
                end_found = True
                print("end found!")
                break
    # if len(queue) > 3:
...
path = []
for point in points:
    if abs(point.x - end_pos[0]) < 20 and abs(point.y - end_pos[1]) < 20 and abs(point.orientation % 360) == end_pos[2]:
        path.append(point)
        break
while True:
    print(len(path))
